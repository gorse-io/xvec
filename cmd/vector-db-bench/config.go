...
func parseConfig(args []string, stderr io.Writer) (benchConfig, error) {
	...
	flags.StringVar(&config.Path, "path", "", "zvec collection path (required)")
	...
	flags.IntVar(&config.OptimizeConcurrency, "optimize-concurrency", 0, "zvec optimize workers; zero uses runtime default")
	...
	flags.StringVar(&config.DBLabel, "db-label", "zvec-go", "label stored in the result")
