...
- Added a checksum-pinned collection fixture generated by the actual v0.1.0
  release and an end-to-end cross-version gate covering immutable segments,
  snapshots, unflushed WAL recovery, deletions, monotonic document IDs, current
  DDL/INVERT migration, Optimize publication, and read-only reopen, together
  with the native format compatibility and upgrade policy.
