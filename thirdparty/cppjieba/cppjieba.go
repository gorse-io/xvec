...
const (
	CutModeSearch CutMode = "search"
	CutModeMix    CutMode = "mix"
	CutModeFull   CutMode = "full"
...
// Options identifies the resource files and cut mode used by a Segmenter.
type Options struct {
...
// Word is one owned segmented term and its original byte offset.
type Word struct {
	Text   string
	Offset uint32
}
...
// New loads the resources required by the selected cut mode.
func New(ctx context.Context, options Options) (*Segmenter, error) {
	if ctx == nil {
	...
	if !mode.valid() {
		return nil, fmt.Errorf("unknown cut mode %q", options.CutMode)
	}
	segmenter := &Segmenter{mode: mode}
	if mode != CutModeHMM {
		dictionary, err := loadCachedJiebaDictionary(ctx, options.DictionaryPath)
		if err != nil {
			return nil, fmt.Errorf("load %s: %w", options.DictionaryPath, err)
		}
		if options.UserDictPath != "" {
			dictionary = dictionary.clone()
			if err := dictionary.loadUserDictionaries(ctx, options.UserDictPath); err != nil {
				return nil, fmt.Errorf("load user dictionary: %w", err)
			}
		}
		segmenter.dictionary = dictionary
	}
	if mode != CutModeFull {
		model, err := loadJiebaHMMModel(ctx, options.HMMModelPath)
		if err != nil {
			return nil, fmt.Errorf("load %s: %w", options.HMMModelPath, err)
		}
...
func (m CutMode) valid() bool {
	switch m {
	case CutModeSearch, CutModeMix, CutModeFull, CutModeHMM:
		return true
	default:
		return false
	}
...
func loadCachedJiebaDictionary(ctx context.Context, path string) (*jiebaDictionary, error) {
	dictionaryCache.Lock()
	if dictionary := dictionaryCache.items[path]; dictionary != nil {
		dictionaryCache.Unlock()
	...
	dictionaryCache.Lock()
	defer dictionaryCache.Unlock()
	if existing := dictionaryCache.items[path]; existing != nil {
		return existing, nil
	}
	dictionaryCache.items[path] = dictionary
	return dictionary, nil
}
...
// Cut segments text with the immutable resources loaded at construction.
// Word offsets refer to the original byte sequence.
func (t *Segmenter) Cut(ctx context.Context, text string) ([]Word, error) {
	if ctx == nil {
		return nil, errors.New("core: nil tokenizer context")
	}
	if err := ctx.Err(); err != nil {
	...
	wordRanges := make([]jiebaWordRange, 0, len(runes))
	for _, inputRange := range ranges {
		...
		wordRanges = append(wordRanges, cut...)
	}
	words := make([]Word, 0, len(wordRanges))
	for index, wordRange := range wordRanges {
		if index&4095 == 0 {
			if err := ctx.Err(); err != nil {
				return nil, err
			}
		}
		if wordRange.start < 0 || wordRange.end < wordRange.start || wordRange.end >= len(runes) {
			return nil, errors.New("core: jieba produced an invalid word range")
		}
		start := runes[wordRange.start].start
		end := runes[wordRange.end].end
		if start == end {
			continue
		}
		words = append(words, Word{Text: strings.Clone(text[start:end]), Offset: start})
	}
	return words, nil
}
...
func (t *Segmenter) cutJiebaMP(ctx context.Context, runes []jiebaRune, start, end int) ([]jiebaWordRange, error) {
...
func (t *Segmenter) cutJiebaFull(ctx context.Context, runes []jiebaRune, start, end int) ([]jiebaWordRange, error) {
	dag, err := t.dictionary.buildDAG(ctx, runes, start, end)
	if err != nil {
		return nil, err
	}
...
func (t *Segmenter) cutJiebaMix(ctx context.Context, runes []jiebaRune, start, end int) ([]jiebaWordRange, error) {
...
func (t *Segmenter) cutJiebaSearch(ctx context.Context, runes []jiebaRune, start, end int) ([]jiebaWordRange, error) {
...
func (t *Segmenter) cutJiebaHMM(ctx context.Context, runes []jiebaRune, start, end int) ([]jiebaWordRange, error) {
...
func (t *Segmenter) cutJiebaHMMInternal(ctx context.Context, runes []jiebaRune, start, end int) ([]jiebaWordRange, error) {
	length := end - start
