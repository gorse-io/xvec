...
import {
  backends, quantizations, textFields, booleanFields, integerFields, decimalFields,
...
export function parseBenchmarks(csv: string, source = 'benchmark-hnsw.csv'): Benchmark[] {
  ...
  return rows.slice(1).map(({ record, info }) => {
    ...
    if (!(quantizations as readonly string[]).includes(String(value.quantize_type))) invalid('quantize_type', 'expected int4, int8, or fp16');
    if (value.index_type !== 'hnsw') invalid('index_type', 'expected hnsw');
