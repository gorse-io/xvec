...
import {
  backends, quantizations, indexTypes, hnswFields, textFields, booleanFields, integerFields, decimalFields,
...
export function parseBenchmarks(csv: string, source = 'benchmark-hnsw.csv'): Benchmark[] {
  ...
  const missing = requiredFields.filter((field) => !(hnswFields as readonly string[]).includes(field) && !header.includes(field));
  ...
  return rows.slice(1).map(({ record, info }) => {
    ...
    for (const field of [...integerFields, ...decimalFields]) {
      if (raw.index_type === 'flat' && (hnswFields as readonly string[]).includes(field) && raw[field] === undefined) continue;
    ...
    for (const field of ['m', 'ef_construction', 'ef_search', 'k', 'batch_size', 'max_docs_per_segment',
    ...
    if (!(indexTypes as readonly string[]).includes(String(value.index_type))) invalid('index_type', 'expected hnsw or flat');
