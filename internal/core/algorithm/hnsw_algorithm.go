...
func (i *HNSWIndex) selectHNSWNeighbors(ctx context.Context, owner int, candidates []hnswScoredNode, limit int) ([]int, error) {
	...
	for candidateIndex, candidate := range candidates {
		...
		for _, accepted := range selected {
			between, err := i.computeDistanceAt(candidate.position, accepted)
			if err != nil {
				return nil, err
			}
...
func (i *HNSWIndex) magnitudeAt(position int) float32 {
	if len(i.vectorMagnitudes) == len(i.keys) {
		return i.vectorMagnitudes[position]
	}
	return 0
}

func (i *HNSWIndex) cacheCosineMagnitudes(ctx context.Context, workers int) error {
...
func decodeHNSWIndex(ctx context.Context, encoded []byte) (*HNSWIndex, error) {
	return decodeHNSWIndexWithVectors(ctx, encoded, nil)
}
...
func decodeHNSWIndexWithVectors(ctx context.Context, encoded []byte, borrowed map[uint64][]float32) (*HNSWIndex, error) {
	...
	for position := 0; position < count; position++ {
		...
		if borrowed != nil {
			...
			if !found || len(vector) != dimension {
				return nil, fmt.Errorf("%w: missing or invalid external vector %d", ErrInvalidHNSWFile, key)
			}
			index.vectorRows[position] = vector[:dimension:dimension]
		}
	...
	if err := index.cacheCosineMagnitudes(ctx, min(max(1, len(index.keys)), 8)); err != nil {
		return nil, fmt.Errorf("%w: cache vector magnitudes: %v", ErrInvalidHNSWFile, err)
	}
	if err := validateHNSWIndex(ctx, index); err != nil {
		return nil, err
	}
...
func validateHNSWIndex(ctx context.Context, index *HNSWIndex) error {
	...
	if index.vectorRows != nil {
		validVectorStorage = !index.fp16 && len(index.vectors) == 0 && len(index.vectorsFP16) == 0 && len(index.vectorRows) == count
		for _, vector := range index.vectorRows {
	...
	if (index.options.Metric == MetricCosine && len(index.vectorMagnitudes) != count) ||
		...
		return fmt.Errorf("%w: inconsistent magnitude storage", ErrInvalidHNSWFile)
	}
	if count == 0 {
