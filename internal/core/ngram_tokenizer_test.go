...
func TestNGramTokenizerBaselineFixture(t *testing.T) {
	...
	if fixture.BaselineCommit != "58375ff7b8fdd0d6fc7d234e47567b179777883b" ||
		fixture.SourceSHA256 != "106f58aa34dc0a3d718e1131656edffcadfca51d5700628287259be2986e7785" ||
		fixture.UnicodeProvider != "utf8proc 2.11.3 e5e799221b45bbb90f5fdc5c69b6b8dfbf017e78" ||
		fixture.UnicodeDataSHA256 != "950e549dbfc853c4304425f3af1875e72fa9fc9697c273c763400c2da4e380a7" ||
...
func TestNGramTokenizerOptions(t *testing.T) {
	...
	if defaults != (NGramTokenizerOptions{Min: 2, Max: 2}) {
	...
	for _, options := range valid {
		...
		if tokenizer.Name() != "ngram" || tokenizer.Min() != options.Min || tokenizer.Max() != options.Max || tokenizer.TokenChars() != options.TokenChars {
	...
	for _, options := range invalid {
		...
		if !errors.Is(err, ErrInvalidNGramTokenizerOptions) {
...
func TestNGramTokenizerPinnedBehavior(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if texts := ngramTokenTexts(got); !reflect.DeepEqual(texts, test.want) {
...
func TestNGramTokenizerNULAndMalformedUTF8(t *testing.T) {
	...
	if want := []string{"a\x00", "\x00b"}; !reflect.DeepEqual(ngramTokenTexts(nulTokens), want) {
		t.Fatalf("NUL tokens = %#v, want %#v", ngramTokenTexts(nulTokens), want)
	}
	...
	if want := []Token{{Text: "ab", Offset: 0, Position: 0}, {Text: "cd", Offset: 3, Position: 1}}; !reflect.DeepEqual(malformedTokens, want) {
		t.Fatalf("malformed tokens = %#v, want %#v", malformedTokens, want)
	}
}
...
func TestNGramTokenizerUnicode17Classes(t *testing.T) {
	...
	if want := []string{"\t", " ", "\n", "\u2028"}; !reflect.DeepEqual(ngramTokenTexts(tokens), want) {
		t.Fatalf("whitespace tokens = %#v, want %#v", ngramTokenTexts(tokens), want)
	}
	...
	if want := []string{"$", "√"}; !reflect.DeepEqual(ngramTokenTexts(tokens), want) {
		t.Fatalf("symbol tokens = %#v, want %#v", ngramTokenTexts(tokens), want)
	}
	...
	if want := []string{"\u1c89", "\u1c8a"}; !reflect.DeepEqual(ngramTokenTexts(tokens), want) {
		t.Fatalf("Unicode 17 letter tokens = %#v, want %#v", ngramTokenTexts(tokens), want)
	}
}
...
func TestNGramTokenizerUnicodeTableIsOrdered(t *testing.T) {
	if len(ngramUnicodeClassRanges) != 1205 {
		t.Fatalf("Unicode class range count = %d, want 1205", len(ngramUnicodeClassRanges))
	}
	for index, item := range ngramUnicodeClassRanges {
		if item.first > item.last || item.last > utf8.MaxRune || item.class == 0 || item.class&^ngramTokenCharAll != 0 {
			t.Fatalf("range %d is invalid: %#v", index, item)
		}
		if index > 0 && ngramUnicodeClassRanges[index-1].last >= item.first {
			t.Fatalf("ranges %d and %d overlap or are unordered", index-1, index)
		}
	}
}
...
func TestNGramTokenizerContextCancellation(t *testing.T) {
	...
	if _, err := tokenizer.Tokenize(midway, strings.Repeat("x", 64<<10)); !errors.Is(err, context.Canceled) {
...
func assertNGramTokenRanges(t testing.TB, input string, tokens []Token, options NGramTokenizerOptions) {
	...
	for position, token := range tokens {
		...
		if token.Position != uint32(position) || token.Text == "" || !utf8.ValidString(token.Text) {
			t.Fatalf("token %d metadata = %#v", position, token)
		}
		if position > 0 && token.Offset < previousOffset {
			t.Fatalf("token %d offset moved backwards: %#v", position, token)
		}
		if end > len(input) || input[start:end] != token.Text {
		...
		if length < options.Min || length > options.Max {
			t.Fatalf("token %d has %d codepoints: %#v", position, length, token)
		}
		if options.TokenChars != 0 {
			for _, value := range token.Text {
				if lookupNGramTokenChar(uint32(value))&options.TokenChars == 0 {
					t.Fatalf("token %d contains excluded codepoint U+%04X", position, value)
				}
			}
		}
