...
// TestDiskANNAtomicSaveProcessKill kills a writer only after its temporary
...
func TestDiskANNAtomicSaveProcessKill(t *testing.T) {
	if testing.Short() {
	...
	command := exec.Command(os.Args[0], "-test.run=^TestDiskANNSaveCrashHelper$")
...
func TestDiskANNSaveCrashHelper(t *testing.T) {
...
// TestDiskANNRecallAndReopenMatrix locks approximate quality and identical
// graph behavior across reopen for every public dense metric.
func TestDiskANNRecallAndReopenMatrix(t *testing.T) {
	...
	for _, metric := range []Metric{MetricL2, MetricIP, MetricCosine, MetricMIPSL2} {
		t.Run(diskANNMetricName(metric), func(t *testing.T) {
			...
			index := buildDiskANNIndex(t, candidates, options)
			path := filepath.Join(t.TempDir(), "recall.diskann")
			{
				err := index.Save(context.Background(), path)
				require.NoError(t, err)
			}
...
// TestDiskANNReadResourceBounds exercises a multi-sector layout through an
// instrumented ReaderAt. Search must respect both the worker limit and the
// pinned 128-sector aggregate batch budget.
func TestDiskANNReadResourceBounds(t *testing.T) {
	options := DefaultDiskANNBuildOptions(MetricL2)
...
// BenchmarkDiskANNSearchQualityComparison compares the dense indexes on one
// corpus and reports recall beside latency and allocation data.
func BenchmarkDiskANNSearchQualityComparison(b *testing.B) {
	...
	path := filepath.Join(b.TempDir(), "benchmark.diskann")
	{
...
func BenchmarkDiskANNBuild(b *testing.B) {
...
func BenchmarkDiskANNCosineBuild(b *testing.B) {
	const dimension = 256
