...
func TestWALSyncEveryBatchesAutomaticSync(t *testing.T) {
	wal, err := CreateWAL(context.Background(), filepath.Join(t.TempDir(), "data.wal"), WALOptions{SyncEvery: 4})
	...
	for index := 0; index < 3; index++ {
		_, err = wal.Append(context.Background(), []byte{byte(index)})
		require.NoError(t, err)
	}
	require.Equal(t, 0, syncCount)
	require.Equal(t, uint64(3), wal.dirtyRecords)

	_, err = wal.Append(context.Background(), []byte{3})
	require.NoError(t, err)
	require.Equal(t, 1, syncCount)
	require.Zero(t, wal.dirtyRecords)
}
...
func TestWALRejectsCorruptionWithoutTruncating(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "file version", mutate: func(data []byte) []byte { binary.LittleEndian.PutUint16(data[8:10], 2); return data }, expected: common.ErrUnsupportedFormatVersion},
