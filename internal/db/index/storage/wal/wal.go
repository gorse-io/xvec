...
func openWAL(ctx context.Context, name string, options WALOptions, readOnly bool) (*WAL, error) {
	...
	if readOnly {
		...
		locked, lockErr = lock.TryRLockContext(ctx, common.FileLockRetryDelay)
	} else {
		locked, lockErr = lock.TryLockContext(ctx, common.FileLockRetryDelay)
	}
	if lockErr != nil {
...
func validateWALFileHeader(header []byte) error {
	...
	if version := binary.LittleEndian.Uint16(header[8:10]); version != walFormatVersion {
		return fmt.Errorf("%w: unsupported WAL version %d", common.ErrUnsupportedFormatVersion, version)
	}
