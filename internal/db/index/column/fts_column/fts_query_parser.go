...
func AnalyzeFTSMatchQuery(ctx context.Context, text string, analyzer tokenizer.FTSAnalyzer, defaultOperator FTSDefaultOperator) (FTSQueryNode, error) {
	if ctx == nil {
...
func ParseFTSQuery(ctx context.Context, query string, analyzer tokenizer.FTSAnalyzer, defaultOperator FTSDefaultOperator) (FTSQueryNode, error) {
	if ctx == nil {
...
func buildFTSQueryAST(ctx context.Context, raw *ftsRawQueryNode, analyzer tokenizer.FTSAnalyzer, defaultOperator FTSDefaultOperator, analyzedTokenCount *int) (FTSQueryNode, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if raw.fieldPrefix {
...
func buildAnalyzedFTSTerm(ctx context.Context, tokens []tokenizer.Token, defaultOperator FTSDefaultOperator) (FTSQueryNode, error) {
	if len(tokens) == 0 {
		return nil, nil
	}
	if len(tokens) == 1 {
...
func analyzeFTSQueryText(ctx context.Context, analyzer tokenizer.FTSAnalyzer, token FTSQueryToken, text string, analyzedTokenCount *int) ([]tokenizer.Token, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	tokens, err := analyzer.Analyze(ctx, text)
