...
func TestSearchFTSBlockMaxMatchesExhaustiveBooleanAndPhraseSearch(t *testing.T) {
	documents := make([][]tokenizer.Token, 640)
	for documentID := range documents {
		...
		tokens := make([]tokenizer.Token, len(terms))
		for position, term := range terms {
			tokens[position] = tokenizer.Token{Text: term, Position: uint32(position)}
		}
...
func TestSearchFTSBlockMaxDoesNotHideNonFiniteScores(t *testing.T) {
	...
	for documentID := range documents {
		...
		if documentID == 200 {
			documents[documentID] = append(documents[documentID], tokenizer.Token{Text: "toxic"})
		}
		for position := range documents[documentID] {
			documents[documentID][position].Position = uint32(position)
		}
...
func repeatedFTSTestTokens(term string, count int) []tokenizer.Token {
	tokens := make([]tokenizer.Token, count)
...
func FuzzSearchFTSBM25(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte, queryMask uint8, requireAll bool, deletionMask uint16, topKByte uint8) {
		...
		documents := make([][]tokenizer.Token, len(data))
		termCounts := make([][4]uint32, len(data))
		for documentID, value := range data {
			tokenCount := int(value%4) + 1
			documents[documentID] = make([]tokenizer.Token, tokenCount)
			for position := range tokenCount {
				termIndex := (int(value) + position) % 4
				documents[documentID][position] = tokenizer.Token{Text: string(rune('a' + termIndex)), Position: uint32(position)}
...
func BenchmarkSearchFTSBM25(b *testing.B) {
	documents := make([][]tokenizer.Token, 10_000)
	for documentID := range documents {
		documents[documentID] = []tokenizer.Token{{Text: "common", Position: 0}, {Text: "term", Position: 1}}
		if documentID%5 == 0 {
			documents[documentID] = append(documents[documentID], tokenizer.Token{Text: "rare", Position: 2})
		}
	}
