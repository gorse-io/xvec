...
func (s *WriteSegment) writeImmutable(ctx context.Context, collectionDir, relativeName string, seal bool) (*ImmutableSegment, error) {
	...
	if err := common.ValidatePortableRelativePath(relativeName); err != nil {
		return nil, err
	}
	if err := ctx.Err(); err != nil {
		return nil, err
...
func OpenImmutableSegment(ctx context.Context, collectionDir string, metadata common.SegmentMetadata) (*ImmutableSegment, error) {
	...
	if err := common.ValidateSegment(metadata); err != nil {
...
func (s *ImmutableSegment) Metadata() common.SegmentMetadata {
	...
	return common.CloneSegment(s.metadata)
}
...
func decodeSegment(ctx context.Context, encoded []byte) (common.SegmentMetadata, []StoredDocument, error) {
	...
	if !bytes.Equal(header[:8], segmentMagic[:]) {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: invalid magic", ErrSegmentCorrupt)
	}
	if version := binary.LittleEndian.Uint16(header[8:10]); version != segmentCodecVersion {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: segment version %d", common.ErrUnsupportedFormatVersion, version)
	}
	if size := binary.LittleEndian.Uint16(header[10:12]); size != segmentHeaderSize {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: invalid header size %d", ErrSegmentCorrupt, size)
	}
	if binary.LittleEndian.Uint32(header[12:16]) != 0 {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: nonzero reserved bytes", ErrSegmentCorrupt)
	}
	if actual, expected := hashutil.CRC32C(header[:60]), binary.LittleEndian.Uint32(header[60:64]); actual != expected {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: header checksum got %08x, want %08x", ErrSegmentCorrupt, actual, expected)
	}
	...
	if payloadLength > maxSegmentPayloadSize || payloadLength != uint64(len(encoded)-segmentHeaderSize) {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: invalid payload length %d", ErrSegmentCorrupt, payloadLength)
	}
	...
	if actual, expected := hashutil.CRC32C(payload), binary.LittleEndian.Uint32(header[56:60]); actual != expected {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: payload checksum got %08x, want %08x", ErrSegmentCorrupt, actual, expected)
	}
	metadata := common.SegmentMetadata{
	...
	if metadata.MaxDocID < metadata.MinDocID || metadata.DocCount-1 != metadata.MaxDocID-metadata.MinDocID {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: non-contiguous document range", ErrSegmentCorrupt)
	}
	docs := make([]StoredDocument, 0, int(metadata.DocCount))
	offset := 0
	for index := uint64(0); index < metadata.DocCount; index++ {
		if err := ctx.Err(); err != nil {
			return common.SegmentMetadata{}, nil, err
		}
		if len(payload)-offset < segmentRecordHeaderSize {
			return common.SegmentMetadata{}, nil, fmt.Errorf("%w: truncated record %d", ErrSegmentCorrupt, index)
		}
		...
		if binary.LittleEndian.Uint32(recordHeader[20:24]) != 0 {
			return common.SegmentMetadata{}, nil, fmt.Errorf("%w: record %d reserved bytes", ErrSegmentCorrupt, index)
		}
		offset += segmentRecordHeaderSize
		remaining := uint64(len(payload) - offset)
		if keyLength == 0 || keyLength > common.MaxPrimaryKeyBytes || documentLength > MaxDocumentPayloadSize || keyLength+documentLength > remaining {
			return common.SegmentMetadata{}, nil, fmt.Errorf("%w: invalid record %d lengths", ErrSegmentCorrupt, index)
		}
		...
		if crc != expectedCRC {
			return common.SegmentMetadata{}, nil, fmt.Errorf("%w: record %d checksum", ErrSegmentCorrupt, index)
		}
		...
		if err := validateStoredDocument(doc, metadata.MinDocID+index); err != nil {
			return common.SegmentMetadata{}, nil, fmt.Errorf("%w: record %d: %v", ErrSegmentCorrupt, index, err)
		}
		docs = append(docs, doc)
	}
	if offset != len(payload) {
		return common.SegmentMetadata{}, nil, fmt.Errorf("%w: trailing segment payload", ErrSegmentCorrupt)
	}
...
func validateStoredDocument(doc StoredDocument, expectedDocID uint64) error {
	...
	if err := common.ValidatePrimaryKey(doc.PrimaryKey); err != nil {
		return err
	}
...
func segmentMetadata(id uint64, docs []StoredDocument, files []string) common.SegmentMetadata {
	metadata := common.SegmentMetadata{ID: id, DocCount: uint64(len(docs)), Files: slices.Clone(files)}
	if len(docs) > 0 {
...
func segmentMetadataEqual(left, right common.SegmentMetadata) bool {
