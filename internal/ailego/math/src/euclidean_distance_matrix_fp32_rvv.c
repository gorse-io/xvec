...
float squared_euclidean_distance_fp32_rvv(const float *lhs, const float *rhs,
                                          int64_t size) {
    ...
    for (int64_t index = 0; index < vectors; index++) {
        ...
        vfloat32m1_t difference = __riscv_vfsub_vv_f32m1(lhs_value, rhs_value, vlmax);
    ...
    if (remain > 0) {
        ...
        vfloat32m1_t difference = __riscv_vfsub_vv_f32m1(lhs_value, rhs_value, vl);
