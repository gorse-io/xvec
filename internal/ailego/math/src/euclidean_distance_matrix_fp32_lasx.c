...
float squared_euclidean_distance_fp32_lasx(const float *lhs, const float *rhs,
                                           long size) {
    long vectors = size / 8;
    long remain = size % 8;
    ...
    for (long index = 0; index < vectors; index++) {
        ...
        __m256 difference = __lasx_xvfsub_s(lhs_value, rhs_value);
    ...
    for (long index = 0; index < remain; index++) {
        float difference = lhs[index] - rhs[index];
